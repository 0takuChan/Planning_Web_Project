...
// Interface สำหรับ step usage
interface StepUsage {
  job_id: number;
  job_number: string;
  customer_name: string;
  has_production_logs: boolean;
  production_logs_count: number;
  production_logs: {
    log_date: string;
    quantity: number;
...
export default function Jobs() {
  ...
  const [stepUsages, setStepUsages] = useState<{[stepId: number]: StepUsage[]}>({});
  ...
  // Fetch step usage สำหรับ job ที่กำลัง edit
  useEffect(() => {
    const fetchStepUsages = async () => {
      ...
      try {
        ...
        const usageMap: {[stepId: number]: StepUsage[]} = {};
        results.forEach(({ stepId, usage }) => {
          usageMap[stepId] = usage;
        });
        
        setStepUsages(usageMap);
      } catch (error) {
        console.error("Failed to fetch step usages:", error);
      }
    };
  ...
  const openEdit = (i: number) => {
    ...
    setEditDraft({...job});
    setStepUsages({}); // Reset step usages
  };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      try {
        ...
        if (existingJobStepsRes.ok) {
          ...
          const deletePromises = jobStepsToDelete.map(async (js: any) => {
            ...
            if (!deleteResponse.ok) {
              ...
              if (errorData.hasProductionLogs) {
        ...
        console.error("Failed to update job steps:", jsError);
        setError(jsError.message || "Failed to update job steps");
        return;
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (errorData.error) {
            ...
            if (errorData.error.includes("referenced by other records") || 
                errorData.error.includes("being used") ||
                errorData.error.includes("มีข้อมูลที่เชื่อมโยงอยู่") ||
                errorData.error.includes("production logs") ||
                errorData.error.includes("Foreign key constraint")) {
              
              const jobNumber = jobs[editIndex].job;
              const customerName = jobs[editIndex].customer;
              

              errorMessage = `ไม่สามารถลบงาน "${jobNumber}" (${customerName}) ได้ เนื่องจากมีข้อมูลที่เชื่อมโยงอยู่ เช่น:\n` +
                           `• บันทึกการผลิต (Production Logs)\n` +
                           `• ข้อมูลการวางแผน (Planning)\n` +
                           `• ขั้นตอนการผลิต (Job Steps)\n\n` +
                           `กรุณาลบข้อมูลที่เกี่ยวข้องเหล่านี้ก่อน หรือติดต่อผู้ดูแลระบบ`;
            } else {
          ...
          // ถ้าแปลง JSON ไม่ได้ ให้ลองอ่านเป็น text
          try {
            const errorText = await response.text();
      ...
      toast({
        title: "Success",
        description: `Job "${jobs[editIndex].job}" deleted successfully`,
      });
      
    } catch (error) {
      console.error("Failed to delete job:", error);
      
      let displayError = "Failed to delete job";
      ...
      // แสดงใน state error และ toast
      setError(displayError);
      
      toast({
        title: "Delete Failed",
        description: displayError.length > 100 ? 
  ...
  const toggleStepSelection = async (stepId: number, isEdit: boolean = false) => {
    if (isEdit) {
      const currentSteps = editDraft.selectedSteps;
      const isRemoving = currentSteps.includes(stepId);
      ...
      if (isRemoving) {
        ...
        try {
          ...
          if (usageResponse.ok) {
            ...
            const jobUsage = usageData.usage.find((u: any) => u.job_id === jobId);
            
            if (jobUsage && jobUsage.has_production_logs) {
              const logDetails = jobUsage.production_logs.map((log: any) => 
                `วันที่ ${new Date(log.log_date).toLocaleDateString('th-TH')} จำนวน ${log.quantity} ชิ้น โดย ${log.employee_name}`
              ).join(", ");
              
              setError(`ไม่สามารถลบ Step "${steps.find(s => s.step_id === stepId)?.step_name}" ได้ เนื่องจากมีบันทึกการผลิต: ${logDetails}`);
              
              toast({
                title: "Cannot Remove Step",
                description: `Step "${steps.find(s => s.step_id === stepId)?.step_name}" has production logs and cannot be removed.`,
                variant: "destructive",
              });
              return;
            }
          ...
          console.warn("Failed to check step usage:", error);
        }
      }
  ...
  const getStepStatus = (stepId: number) => {
    const usage = stepUsages[stepId];
    if (!usage || usage.length === 0) return 'available';
    
    const hasLogs = usage.some(u => u.has_production_logs);
    return hasLogs ? 'locked' : 'available';
  };
  ...
  return (
    <AppLayout>
      ...
      <Dialog open={editIndex !== null} onOpenChange={(o) => { if (!o) { setEditIndex(null); setStepUsages({}); } }}>
        <DialogContent className="max-w-4xl p-0 overflow-hidden">
          <div className="grid grid-cols-1 md:grid-cols-2">
            <div className="p-6">
              <DialogHeader>
              ...
              <div className="mt-4 space-y-4 text-sm">
                ...
                <div className="mt-6 flex justify-between gap-3">
                  ...
                  <div className="flex gap-3">
                    <Button variant="outline" onClick={() => { setEditIndex(null); setStepUsages({}); }}>
                      Cancel
                    </Button>
                    <Button
            ...
            <div className="p-6 bg-slate-50 md:bg-gradient-to-br md:from-[hsl(var(--brand-start))] md:to-[hsl(var(--brand-end))]">
              <div className="rounded-lg border bg-white/90 backdrop-blur p-4 shadow">
                <div className="font-medium mb-2">Production Steps</div>
                <div className="text-xs text-slate-500 mb-3">
                  Modify the steps required for this job. Steps with production logs cannot be removed.
                </div>
                <div className="space-y-2 text-sm max-h-80 overflow-y-auto">
                  {steps.map((step) => {
                    const isSelected = editDraft.selectedSteps.includes(step.step_id);
                    const stepStatus = getStepStatus(step.step_id);
                    const isLocked = stepStatus === 'locked';
                    const usage = stepUsages[step.step_id];
                    const hasLogs = usage && usage.some(u => u.has_production_logs);
