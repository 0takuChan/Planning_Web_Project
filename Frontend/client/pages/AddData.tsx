                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      <AlertDialogTitle className="flex items-center gap-2">
                        ...
                        Delete Production Log Confirmation
                      </AlertDialogTitle>
                      <AlertDialogDescription className="space-y-3">
                        <div>
                          Are you sure you want to delete this production log? This action cannot be undone.
                        </div>
                        {editIdx !== null && (
                          <div className="bg-gray-50 p-3 rounded-lg border">
                            <div className="text-sm font-medium text-gray-900">Production Log Details:</div>
                            <div className="text-sm text-gray-600 mt-1 space-y-1">
                              <div><strong>Job:</strong> {editDraft.job_display || `Job ${productionLogs[editIdx]?.job_id}`}</div>
                              <div><strong>Step:</strong> {editDraft.step_display || "Unknown Step"}</div>
                              <div><strong>Quantity:</strong> {editDraft.quantity}</div>
                              <div><strong>Date:</strong> {editDraft.log_date}</div>
                              <div><strong>Employee:</strong> {productionLogs[editIdx]?.employee?.fullname || `ID: ${productionLogs[editIdx]?.employee_id}`}</div>
                            </div>
                          </div>
                        )}
                        <div className="text-red-600 text-sm font-medium">
                          ⚠️ This will permanently remove the production log from the system.
                        </div>
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
