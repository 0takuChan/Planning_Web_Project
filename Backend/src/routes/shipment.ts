...
type ShipmentWithActualDelivery = Shipment & {
...
const getJobShipmentCapacity = async (jobId: number, excludeShipmentId?: number) => {
  ...
  const shipmentAggregate = await prisma.shipment.aggregate({
    where: {
      job_id: jobId,
      ...(excludeShipmentId ? { shipment_id: { not: excludeShipmentId } } : {}),
    },
    _sum: {
      total_quantity: true,
    },
  ...
  const shippedQuantity = shipmentAggregate._sum.total_quantity ?? 0;
...
router.get("/", async (_req: Request, res: Response) => {
  ...
  const result = (shipments as ShipmentWithActualDelivery[]).map((s) => ({
    ...s,
...
router.get("/:id", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const typedShipment = shipment as ShipmentWithActualDelivery;

    const result = {
      ...typedShipment,
      calculated_status: calculateShipmentStatus(
        typedShipment.departure_date,
        typedShipment.arrival_date,
        typedShipment.actual_delivery_date
      ),
    };
...
router.post("/", async (req: Request, res: Response) => {
  try {
    ...
    const shipmentCapacity = await getJobShipmentCapacity(Number(job_id));
...
router.put("/:id", async (req: Request<{ id: string }>, res: Response) => {
  ...
  try {
    ...
    const parsedTotalQuantity = Object.prototype.hasOwnProperty.call(req.body, "total_quantity")
      ...
      : undefined;
    const targetJobId = req.body.job_id ?? existingShipment.job_id;
    ...
    if (parsedTotalQuantity !== undefined || req.body.job_id !== undefined) {
      const shipmentCapacity = await getJobShipmentCapacity(Number(targetJobId), id);
      ...
      const requestedQuantity = parsedTotalQuantity ?? existingShipment.total_quantity;
