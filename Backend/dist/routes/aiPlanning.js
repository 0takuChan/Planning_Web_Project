...
const geminiPlanning_1 = require("../services/geminiPlanning");
...
function getUsedMinutesForStepOnDate(stepId, plannedDate) {
...
function getExistingStepMinutesByDate(stepIds_1, startDate_1, endDate_1) {
    return __awaiter(this, arguments, void 0, function* (stepIds, startDate, endDate, excludeJobIds = []) {
        if (!stepIds.length) {
            return {};
        }
        const plannings = yield prisma.planning.findMany({
            where: Object.assign(Object.assign({ planned_date: Object.assign({ gte: parseDateOnlyUtc(startDate) }, (endDate ? { lte: endOfDateOnlyUtc(endDate) } : {})) }, (excludeJobIds.length
                ? {
                    job_id: {
                        notIn: excludeJobIds,
                    },
        ...
        return plannings.reduce((usageMap, planning) => {
...
function buildGeminiErrorResponse(error) {
    const message = (error === null || error === void 0 ? void 0 : error.message) || "An error occurred while generating the plan";
    ...
    if (message.includes("Gemini API quota exhausted") ||
    ...
    if (message.includes("Gemini API Rate Limited") ||
...
function createPlanningRecords(planningPairs) {
    return __awaiter(this, void 0, void 0, function* () {
        const createdPlannings = [];
        let successCount = 0;
        let skippedCount = 0;
        const jobStepCache = new Map();
        for (const pair of planningPairs) {
            try {
                let jobStep = jobStepCache.get(pair.job_step_id);
                if (!jobStep) {
                    jobStep = yield prisma.jobStep.findUnique({
                ...
                while (remainingQuantity > 0) {
                    ...
                    debugLog(`  [AUTO-PLAN API] Materializing Planning: job_id=${pair.job_id}, step_id=${pair.job_step_id}, date=${targetDate}, remaining_qty=${remainingQuantity}`);
                    if (jobStep.minutes_per_unit && jobStep.minutes_per_unit > 0) {
                        const usedMinutes = yield getUsedMinutesForStepOnDate(jobStep.step_id, targetDate);
                        ...
                        if (availableUnits <= 0) {
                            debugLog(`  [AUTO-PLAN API] Day ${targetDate} for step ${jobStep.step.step_name} is full (${usedMinutes}/${jobStep.step.standard_time} min). Moving to next available day.`);
                        ...
                        debugLog(`  [AUTO-PLAN API] ✓ Saved ${quantityForDate} units on ${targetDate} (${remainingMinutes} free min before save, max ${availableUnits} units)`);
                ...
                if (error.code === "P2002") {
                    debugLog(`  [AUTO-PLAN API] ⚠ Skipping duplicate planning: job_id=${pair.job_id}, job_step_id=${pair.job_step_id}, date=${pair.date}`);
                    skippedCount++;
                }
                else {
 ...
 * }
 */
router.post("/auto-plan", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const stepsWithoutMinutes = jobSteps.filter((js) => js.minutes_per_unit === null || js.minutes_per_unit === undefined);
        ...
        const existingStepMinutesByDate = yield getExistingStepMinutesByDate(stepIds, todayString, undefined, [job_id]);
        const jobStepsWithRemaining = jobSteps
            .map((js) => ({
        ...
        const planningPairs = yield (0, geminiPlanning_1.generateAutoPlan)(job.job_number, dueDateString, jobStepsWithRemaining, existingStepMinutesByDate);
        ...
        const clearedExistingPlans = yield prisma.planning.deleteMany({
            where: { job_id },
        });
        ...
        const { createdPlannings, successCount, skippedCount } = yield createPlanningRecords(planningPairs.map((pair) => ({
            job_id,
            job_step_id: pair.job_step_id,
            date: pair.date,
        ...
        return res.status(200).json({
            success: true,
            message: `Auto plan generated successfully for ${job.job_number}. Created ${successCount} planning records${skippedCount > 0 ? ` (${skippedCount} duplicates skipped)` : ""}.`,
            count: successCount,
...
router.post("/auto-plan-batch", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const todayString = formatLocalDate(new Date());
        const [jobs, jobSteps] = yield Promise.all([
            prisma.job.findMany({
        ...
        const existingStepMinutesByDate = yield getExistingStepMinutesByDate(stepIds, todayString, undefined, uniqueJobIds);
        const failedJobs = [];
        const batchJobs = [];
        for (const jobId of uniqueJobIds) {
            ...
            const stepsWithoutMinutes = stepsForJob.filter((jobStep) => jobStep.minutes_per_unit === null || jobStep.minutes_per_unit === undefined);
            if (stepsWithoutMinutes.length > 0) {
                failedJobs.push(`${job.job_number}: steps missing minutes_per_unit (${stepsWithoutMinutes
                    .map((jobStep) => jobStep.step.step_name)
                    .join(", ")})`);
                continue;
            }
            const stepsWithRemaining = stepsForJob
                .map((jobStep) => {
                return {
        ...
        const batchPlanResult = yield (0, geminiPlanning_1.generateBatchAutoPlan)(batchJobs, existingStepMinutesByDate);
        ...
        debugLog(`[AUTO-PLAN BATCH API] SUCCESS: Created ${successCount} planning records for ${jobsWithPlans.size}/${uniqueJobIds.length} jobs\n`);
        return res.status(200).json({
            success: true,
            message: `Auto plan batch generated for ${jobsWithPlans.size}/${uniqueJobIds.length} jobs. Created ${successCount} planning records${skippedCount > 0 ? ` (${skippedCount} duplicates skipped)` : ""}.`,
            count: successCount,
