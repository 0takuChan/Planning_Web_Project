...
exports.generateAutoPlan = generateAutoPlan;
exports.generateBatchAutoPlan = generateBatchAutoPlan;
...
function isPreferredWorkingDay(dateString) {
...
function getPreferredWorkingDateStrings(startDate, endDate) {
    return getDateStringsInRange(startDate, endDate).filter(isPreferredWorkingDay);
}
function getWeekendDateStrings(startDate, endDate) {
...
function getLaterDateString(left, right) {
...
function getAiProvider() {
    var _a;
    const configuredProvider = (_a = process.env.AI_PROVIDER) === null || _a === void 0 ? void 0 : _a.trim().toLowerCase();
...
function extractRetryDelaySeconds(errorData, provider) {
    var _a, _b, _c, _d, _e;
    const retryAfter = provider === "gemini"
        ? (Array.isArray((_a = errorData === null || errorData === void 0 ? void 0 : errorData.error) === null || _a === void 0 ? void 0 : _a.details)
            ? (_b = errorData.error.details.find((detail) => detail === null || detail === void 0 ? void 0 : detail.retryDelay)) === null || _b === void 0 ? void 0 : _b.retryDelay
            : undefined)
        : (((_c = errorData === null || errorData === void 0 ? void 0 : errorData.error) === null || _c === void 0 ? void 0 : _c.retry_after) || ((_d = errorData === null || errorData === void 0 ? void 0 : errorData.error) === null || _d === void 0 ? void 0 : _d.retryAfter) || (errorData === null || errorData === void 0 ? void 0 : errorData.retry_after));
    const message = String(((_e = errorData === null || errorData === void 0 ? void 0 : errorData.error) === null || _e === void 0 ? void 0 : _e.message) || (errorData === null || errorData === void 0 ? void 0 : errorData.message) || "");
    let waitSeconds = 32;
...
function isPermanentQuotaExhaustion(errorData, provider) {
    var _a, _b, _c;
    const message = String(((_a = errorData === null || errorData === void 0 ? void 0 : errorData.error) === null || _a === void 0 ? void 0 : _a.message) || (errorData === null || errorData === void 0 ? void 0 : errorData.message) || "");
    if (provider === "groq") {
        return /limit:\s*0\b/i.test(message);
    }
    const violations = Array.isArray((_b = errorData === null || errorData === void 0 ? void 0 : errorData.error) === null || _b === void 0 ? void 0 : _b.details)
        ? errorData.error.details.flatMap((detail) => Array.isArray(detail === null || detail === void 0 ? void 0 : detail.violations) ? detail.violations : [])
        : [];
    return (((_c = errorData === null || errorData === void 0 ? void 0 : errorData.error) === null || _c === void 0 ? void 0 : _c.status) === "RESOURCE_EXHAUSTED" &&
        (/limit:\s*0\b/i.test(message) ||
            violations.some((violation) => String((violation === null || violation === void 0 ? void 0 : violation.quotaId) || "").includes("PerDay"))));
}
...
function sanitizeJsonCandidate(jsonText) {
    return jsonText
        .trim()
        .replace(/^```(?:json)?\s*/i, "")
        .replace(/\s*```$/i, "")
...
function extractJsonArrayCandidate(responseText) {
...
function recoverParsableObjects(jsonCandidate) {
    const recovered = [];
    ...
    for (let index = arrayStart >= 0 ? arrayStart : 0; index < normalized.length; index += 1) {
        ...
        if (char === "}" && braceDepth > 0) {
            ...
            if (braceDepth === 0 && objectStart >= 0) {
                ...
                try {
                    recovered.push(JSON.parse(sanitizeJsonCandidate(objectText)));
                }
                catch (_a) {
...
function parseAiJsonArray(responseText, contextLabel) {
    ...
    try {
        ...
        const recovered = recoverParsableObjects(jsonCandidate);
        if (recovered.length > 0) {
            console.warn(`${contextLabel}: Recovered ${recovered.length} complete object(s) from malformed AI JSON response.`);
            return recovered;
        }
        throw error;
    }
...
function retryFetch(url_1, options_1) {
    return __awaiter(this, arguments, void 0, function* (url, options, maxRetries = 3, provider = "gemini") {
        let lastError = null;
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (response.status === 429) {
                    const errorData = yield readErrorResponse(response);
                ...
                if (!response.ok) {
                    ...
                    if (response.status === 401 ||
                        response.status === 403 ||
                        (response.status === 400 && (errorMessage.includes("API_KEY_INVALID") ||
...
function callAiTextResponse(provider, prompt, requestLabel) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (provider === "groq") {
            ...
            const response = yield retryFetch(apiUrl, {
                method: "POST",
                headers: {
            ...
            const responseText = (_c = (_b = (_a = result === null || result === void 0 ? void 0 : result.choices) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.message) === null || _c === void 0 ? void 0 : _c.content;
        ...
        debugLog("Full Response:");
        debugLog(JSON.stringify(result, null, 2));
        const responseText = (_h = (_g = (_f = (_e = (_d = result === null || result === void 0 ? void 0 : result.candidates) === null || _d === void 0 ? void 0 : _d[0]) === null || _e === void 0 ? void 0 : _e.content) === null || _f === void 0 ? void 0 : _f.parts) === null || _g === void 0 ? void 0 : _g[0]) === null || _h === void 0 ? void 0 : _h.text;
        if (!responseText || typeof responseText !== "string") {
            debugLog(`ERROR: Invalid ${providerName} response structure`);
            throw new Error(`Invalid ${providerName} API response structure`);
        }
...
function mergePlanningPairs(plans) {
    ...
    for (const plan of plans) {
        ...
        if (existing) {
            ...
            merged.set(key, Object.assign({}, plan));
        }
    }
...
function mergeBatchPlanningPairs(plans) {
    const merged = new Map();
    for (const plan of plans) {
...
function getUsedMinutesForStepOnDate(usageMap, stepId, date) {
    return usageMap.get(`${stepId}:${date}`) || 0;
}
function getAvailableUnitsForDate(step, date, usageMap) {
...
function addStepMinutesUsage(usageMap, stepId, date, minutesToAdd) {
    const key = `${stepId}:${date}`;
    usageMap.set(key, (usageMap.get(key) || 0) + minutesToAdd);
}
function getImmediatePredecessorSteps(currentStep, allSteps) {
...
function getCumulativeAllocatedQuantity(allocationMap, currentStep, productionDates, targetDate, includeTargetDate, keyBuilder) {
...
function getPrecedenceAllowanceForDate(currentStep, allSteps, allocationMap, productionDates, targetDate, keyBuilder) {
    const predecessorSteps = getImmediatePredecessorSteps(currentStep, allSteps);
    if (predecessorSteps.length === 0) {
        return Number.POSITIVE_INFINITY;
    }
    const currentAllocatedBeforeDate = getCumulativeAllocatedQuantity(allocationMap, currentStep, productionDates, targetDate, false, keyBuilder);
    const predecessorCompletionLimit = predecessorSteps.reduce((minimumAllowed, predecessorStep) => {
        const predecessorAllocatedBeforeDate = getCumulativeAllocatedQuantity(allocationMap, predecessorStep, productionDates, targetDate, false, keyBuilder);
...
function extendProductionDatesUntilPlanned(currentStep, allSteps, allocationMap, usedMinutesMap, allocatedByJobStep, productionDates, remaining, extensionStartDate, keyBuilder, logLabel) {
    ...
    while (unresolved > 0 && extraDays < 365) {
        ...
        const precedenceAllowance = getPrecedenceAllowanceForDate(currentStep, allSteps, allocationMap, productionDates, nextDate, keyBuilder);
...
function extendBatchProductionDatesUntilPlanned(jobId, jobNumber, currentStep, allSteps, allocationMap, usedMinutesMap, allocatedByJobStep, productionDates, remaining, extensionStartDate) {
    let unresolved = remaining;
    let anchorDate = productionDates[productionDates.length - 1] || "";
    let extraDays = 0;
    while (unresolved > 0 && extraDays < 365) {
        ...
        const usedMinutes = getUsedMinutesForStepOnDate(usedMinutesMap, currentStep.step_id, nextDate);
        const remainingMinutes = Math.max(0, currentStep.standard_time - usedMinutes);
        const precedenceAllowance = getPrecedenceAllowanceForDate(currentStep, allSteps, allocationMap, productionDates, nextDate, (step, date) => `${jobId}:${step.job_step_id}:${date}`);
        if (availableCapacity <= 0 || precedenceAllowance <= 0) {
...
function rebalanceSingleStepSingletonDays(allocationMap, usedMinutesMap, stepCapacities, productionDates) {
    for (const step of stepCapacities) {
        for (const sourceDate of productionDates) {
            ...
            for (const targetDate of targetDates) {
                ...
                debugLog(`[AUTO-PLAN DEBUG] Step ${step.step_name}: moved 1 unit from ${sourceDate} to ${targetDate} to avoid a singleton production day`);
                break;
            }
        }
    }
}
function rebalanceBatchStepSingletonDays(allocationMap, usedMinutesMap, jobs, todayString) {
    for (const job of jobs) {
        ...
        for (const step of job.job_steps.filter((item) => item.minutes_per_unit && item.minutes_per_unit > 0)) {
            const normalizedStep = Object.assign(Object.assign({}, step), { minutes_per_unit: step.minutes_per_unit });
            for (const sourceDate of productionDates) {
                ...
                for (const targetDate of targetDates) {
                    ...
                    debugLog(`[AUTO-PLAN BATCH DEBUG] Job ${job.job_number} step ${step.step_name}: moved 1 unit from ${sourceDate} to ${targetDate} to avoid a singleton production day`);
                    break;
                }
            }
        }
...
function ensureSingleStepCoverage(plans, stepCapacities, todayString, dueDate, existingStepMinutesByDate) {
    const mergedPlans = mergePlanningPairs(plans);
    const allocationMap = new Map();
    const usedMinutesMap = new Map(Object.entries(existingStepMinutesByDate));
    const stepByJobStepId = new Map(stepCapacities.map((step) => [step.job_step_id, step]));
    const allocatedByJobStep = new Map();
    ...
    const sortedMergedPlans = [...mergedPlans].sort((left, right) => {
        ...
        const leftPriority = (_b = (_a = stepByJobStepId.get(left.job_step_id)) === null || _a === void 0 ? void 0 : _a.priority) !== null && _b !== void 0 ? _b : Number.MAX_SAFE_INTEGER;
        const rightPriority = (_d = (_c = stepByJobStepId.get(right.job_step_id)) === null || _c === void 0 ? void 0 : _c.priority) !== null && _d !== void 0 ? _d : Number.MAX_SAFE_INTEGER;
    ...
    for (const plan of sortedMergedPlans) {
        ...
        const precedenceAllowance = getPrecedenceAllowanceForDate(step, stepCapacities, allocationMap, productionDates, plan.date, (currentStep, date) => `${currentStep.job_step_id}:${date}`);
        const adjustedQuantity = Math.min(plan.quantity, availableUnits, remainingAllowed, precedenceAllowance);
        if (adjustedQuantity <= 0) {
            debugLog(`[AUTO-PLAN DEBUG] Step ${step.step_name} ${plan.date}: AI allocation skipped because no shared capacity or predecessor output is available`);
            continue;
        }
        if (adjustedQuantity !== plan.quantity) {
            debugLog(`[AUTO-PLAN DEBUG] Step ${step.step_name} ${plan.date}: AI allocation adjusted from ${plan.quantity} to ${adjustedQuantity} due to shared capacity or predecessor handoff limits`);
        }
    ...
    for (const step of stepCapacities) {
        const maxPossibleBeforeDueDate = beforeDueDates.reduce((sum, date) => sum + getAvailableUnitsForDate(step, date, usedMinutesMap), 0) + (allocatedByJobStep.get(step.job_step_id) || 0);
        debugLog(`[AUTO-PLAN DEBUG] Step ${step.step_name} (${step.job_step_id}) capacity: ${step.standard_time} min/day, ${step.minutes_per_unit} min/unit, max ${maxPossibleBeforeDueDate} units before ${dueDate}`);
        const initialAllocations = productionDates.map((date) => ({
            date,
            quantity: allocationMap.get(`${step.job_step_id}:${date}`) || 0,
        }));
        debugLog(`[AUTO-PLAN DEBUG] Step ${step.step_name} initial allocations: ${initialAllocations
        ...
        debugLog(`[AUTO-PLAN DEBUG] Step ${step.step_name} allocated ${allocated}/${step.remaining_quantity}, remaining ${remaining}`);
        for (const date of productionDates) {
            ...
            const precedenceAllowance = getPrecedenceAllowanceForDate(step, stepCapacities, allocationMap, productionDates, date, (currentStep, currentDate) => `${currentStep.job_step_id}:${currentDate}`);
            if (availableCapacity <= 0) {
                debugLog(`[AUTO-PLAN DEBUG] Step ${step.step_name} ${date}: no free shared capacity left (${usedMinutes}/${step.standard_time} min used)`);
                continue;
            }
            if (precedenceAllowance <= 0) {
                debugLog(`[AUTO-PLAN DEBUG] Step ${step.step_name} ${date}: waiting for predecessor steps to finish enough quantity before handing off more work`);
                continue;
            }
            ...
            debugLog(`[AUTO-PLAN DEBUG] Step ${step.step_name} ${date}: added ${addedQuantity}, day total ${currentQuantity + addedQuantity}, remaining ${remaining}`);
        }
        for (const date of weekendDatesBeforeDue) {
            ...
            allocatedByJobStep.set(step.job_step_id, (allocatedByJobStep.get(step.job_step_id) || 0) + addedQuantity);
            remaining -= addedQuantity;
            debugLog(`[AUTO-PLAN DEBUG] Step ${step.step_name} ${date}: weekend fallback added ${addedQuantity}, day total ${currentQuantity + addedQuantity}, remaining ${remaining}`);
        }
        if (remaining > 0) {
            debugLog(`[AUTO-PLAN DEBUG] Step ${step.step_name}: due-date capacity exhausted, extending planning beyond ${dueDate}`);
            remaining = extendProductionDatesUntilPlanned(step, stepCapacities, allocationMap, usedMinutesMap, allocatedByJobStep, productionDates, remaining, getLaterDateString(todayString, dueDate), (currentStep, currentDate) => `${currentStep.job_step_id}:${currentDate}`, `[AUTO-PLAN DEBUG] Step ${step.step_name}`);
        }
        const finalAllocations = productionDates.map((date) => ({
            date,
            quantity: allocationMap.get(`${step.job_step_id}:${date}`) || 0,
        }));
        debugLog(`[AUTO-PLAN DEBUG] Step ${step.step_name} final allocations: ${finalAllocations
            .map((entry) => `${entry.date}=${entry.quantity}`)
            .join(", ")}`);
        if (remaining > 0) {
            throw new Error(`Unable to create a complete plan for step ${step.step_name}. Missing ${remaining} units even after extending beyond due date ${dueDate}. Shared step capacity and predecessor constraints do not allow enough output, and the maximum possible before due date is ${maxPossibleBeforeDueDate} units.`);
        }
    }
...
function ensureBatchStepCoverage(plans, jobs, todayString, existingStepMinutesByDate) {
    const mergedPlans = mergeBatchPlanningPairs(plans);
    const allocationMap = new Map();
    const usedMinutesMap = new Map(Object.entries(existingStepMinutesByDate));
    const allocatedByJobStep = new Map();
    const overdueRemainingByJobStep = new Map();
    const stepMap = new Map(jobs.flatMap((job) => job.job_steps.map((step) => [
        `${job.job_id}:${step.job_step_id}`,
        Object.assign({ job_id: job.job_id, job_number: job.job_number, due_date: job.due_date }, step),
    ])));
    const jobStepsByJobId = new Map(jobs.map((job) => [job.job_id, job.job_steps]));
    const productionDatesByJobId = new Map(jobs.map((job) => [job.job_id, getPreferredWorkingDateStrings(todayString, job.due_date)]));
    const beforeDueDatesByJobId = new Map(jobs.map((job) => [job.job_id, getPreferredWorkingDateStrings(todayString, job.due_date)]));
    const weekendDatesByJobId = new Map(jobs.map((job) => [job.job_id, getWeekendDateStrings(todayString, job.due_date)]));
    const sortedMergedPlans = [...mergedPlans].sort((left, right) => {
        var _a, _b, _c, _d;
        if (left.date !== right.date) {
            return left.date.localeCompare(right.date);
        ...
        const leftPriority = (_b = (_a = stepMap.get(`${left.job_id}:${left.job_step_id}`)) === null || _a === void 0 ? void 0 : _a.priority) !== null && _b !== void 0 ? _b : Number.MAX_SAFE_INTEGER;
        const rightPriority = (_d = (_c = stepMap.get(`${right.job_id}:${right.job_step_id}`)) === null || _c === void 0 ? void 0 : _c.priority) !== null && _d !== void 0 ? _d : Number.MAX_SAFE_INTEGER;
        if (leftPriority !== rightPriority) {
            return leftPriority - rightPriority;
        }
        return left.job_step_id - right.job_step_id;
    ...
    for (const plan of sortedMergedPlans) {
        ...
        const precedenceAllowance = getPrecedenceAllowanceForDate(normalizedStep, jobStepsByJobId.get(plan.job_id) || [], allocationMap, productionDatesByJobId.get(plan.job_id) || [], plan.date, (currentStep, date) => `${plan.job_id}:${currentStep.job_step_id}:${date}`);
        const adjustedQuantity = Math.min(plan.quantity, availableUnits, remainingAllowed, precedenceAllowance);
        if (adjustedQuantity <= 0) {
            debugLog(`[AUTO-PLAN BATCH DEBUG] Job ${normalizedStep.job_number} step ${normalizedStep.step_name} ${plan.date}: AI allocation skipped because no shared capacity or predecessor output is available`);
            continue;
        }
        if (adjustedQuantity !== plan.quantity) {
            debugLog(`[AUTO-PLAN BATCH DEBUG] Job ${normalizedStep.job_number} step ${normalizedStep.step_name} ${plan.date}: AI allocation adjusted from ${plan.quantity} to ${adjustedQuantity} due to shared capacity or predecessor handoff limits`);
        }
    ...
    for (const job of jobs) {
        ...
        debugLog(`[AUTO-PLAN BATCH DEBUG] Job ${job.job_number} working dates considered: ${productionDates.join(", ")}`);
        for (const step of job.job_steps.filter((item) => item.minutes_per_unit && item.minutes_per_unit > 0)) {
            ...
            const maxPossibleBeforeDueDate = beforeDueDates.reduce((sum, date) => sum + getAvailableUnitsForDate(step, date, usedMinutesMap), 0) + (allocatedByJobStep.get(stepAllocationKey) || 0);
            debugLog(`[AUTO-PLAN BATCH DEBUG] Job ${job.job_number} step ${step.step_name} (${step.job_step_id}) capacity: ${step.standard_time} min/day, ${step.minutes_per_unit} min/unit, max ${maxPossibleBeforeDueDate} units before ${job.due_date}`);
            const initialAllocations = productionDates.map((date) => ({
                date,
                quantity: allocationMap.get(`${job.job_id}:${step.job_step_id}:${date}`) || 0,
            }));
            debugLog(`[AUTO-PLAN BATCH DEBUG] Job ${job.job_number} step ${step.step_name} initial allocations: ${initialAllocations
                .map((entry) => `${entry.date}=${entry.quantity}`)
                .join(", ")}`);
            const allocated = allocatedByJobStep.get(stepAllocationKey) || 0;
            let remaining = step.remaining_quantity - allocated;
            debugLog(`[AUTO-PLAN BATCH DEBUG] Job ${job.job_number} step ${step.step_name} allocated ${allocated}/${step.remaining_quantity}, remaining ${remaining}`);
            for (const date of productionDates) {
                if (remaining <= 0) {
                    break;
                }
                ...
                const usedMinutes = getUsedMinutesForStepOnDate(usedMinutesMap, step.step_id, date);
                const availableCapacity = getAvailableUnitsForDate(step, date, usedMinutesMap);
                const precedenceAllowance = getPrecedenceAllowanceForDate(step, job.job_steps, allocationMap, productionDates, date, (currentStep, currentDate) => `${job.job_id}:${currentStep.job_step_id}:${currentDate}`);
                if (availableCapacity <= 0) {
                    debugLog(`[AUTO-PLAN BATCH DEBUG] Job ${job.job_number} step ${step.step_name} ${date}: no free shared capacity left (${usedMinutes}/${step.standard_time} min used)`);
                    continue;
                }
                if (precedenceAllowance <= 0) {
                    debugLog(`[AUTO-PLAN BATCH DEBUG] Job ${job.job_number} step ${step.step_name} ${date}: waiting for predecessor steps to finish enough quantity before handing off more work`);
                    continue;
                }
                const addedQuantity = Math.min(remaining, availableCapacity, precedenceAllowance);
                const currentQuantity = allocationMap.get(key) || 0;
                ...
                allocatedByJobStep.set(stepAllocationKey, (allocatedByJobStep.get(stepAllocationKey) || 0) + addedQuantity);
                remaining -= addedQuantity;
                debugLog(`[AUTO-PLAN BATCH DEBUG] Job ${job.job_number} step ${step.step_name} ${date}: added ${addedQuantity}, day total ${currentQuantity + addedQuantity}, remaining ${remaining}`);
            }
            for (const date of weekendDates) {
                ...
                allocatedByJobStep.set(stepAllocationKey, (allocatedByJobStep.get(stepAllocationKey) || 0) + addedQuantity);
                remaining -= addedQuantity;
                debugLog(`[AUTO-PLAN BATCH DEBUG] Job ${job.job_number} step ${step.step_name} ${date}: weekend fallback added ${addedQuantity}, day total ${currentQuantity + addedQuantity}, remaining ${remaining}`);
            }
            if (remaining > 0) {
                debugLog(`[AUTO-PLAN BATCH DEBUG] Job ${job.job_number} step ${step.step_name}: due-date capacity exhausted, deferring ${remaining} units to overdue phase so jobs that can still finish on time use shared capacity first`);
            ...
            debugLog(`[AUTO-PLAN BATCH DEBUG] Job ${job.job_number} step ${step.step_name} final allocations: ${finalAllocations
                .map((entry) => `${entry.date}=${entry.quantity}`)
                .join(", ")}`);
            if (remaining <= 0) {
    ...
    for (const job of jobs) {
        ...
        for (const step of job.job_steps.filter((item) => item.minutes_per_unit && item.minutes_per_unit > 0)) {
            ...
            debugLog(`[AUTO-PLAN BATCH DEBUG] Job ${job.job_number} step ${step.step_name}: starting overdue phase with ${remaining} units after all on-time jobs were allocated`);
            const unresolved = extendBatchProductionDatesUntilPlanned(job.job_id, job.job_number, step, job.job_steps, allocationMap, usedMinutesMap, allocatedByJobStep, productionDates, remaining, getLaterDateString(todayString, job.due_date));
            if (unresolved > 0) {
                const maxPossibleBeforeDueDate = (beforeDueDatesByJobId.get(job.job_id) || productionDates).reduce((sum, date) => sum + getAvailableUnitsForDate(step, date, usedMinutesMap), 0) + ((allocatedByJobStep.get(stepAllocationKey) || 0) - unresolved);
                throw new Error(`Unable to create a complete plan for job ${job.job_number} step ${step.step_name}. Missing ${unresolved} units even after extending beyond due date ${job.due_date}. Shared step capacity and predecessor constraints do not allow enough output, and the maximum possible before due date is ${maxPossibleBeforeDueDate} units.`);
            }
        }
    }
...
function generateAutoPlan(jobNumber_1, dueDate_1, jobSteps_1) {
    return __awaiter(this, arguments, void 0, function* (jobNumber, dueDate, jobSteps, existingStepMinutesByDate = {}) {
        ...
        const stepCapacities = orderedJobSteps
            ...
            .map((s) => {
            const minutesPerUnit = s.minutes_per_unit;
            return {
        ...
        if (stepCapacities.length === 0) {
            throw new Error("No job steps with minutes_per_unit configured found");
        }
        const completedPlans = ensureSingleStepCoverage([], stepCapacities.map((step) => ({
        ...
        })), todayString, dueDate, existingStepMinutesByDate);
        const totalPlanned = completedPlans.reduce((sum, plan) => sum + plan.quantity, 0);
        debugLog(`Deterministic weekday-first planning: Total ${totalPlanned} units planned across ${completedPlans.length} planning records`);
...
function generateBatchAutoPlan(jobs_1) {
    return __awaiter(this, arguments, void 0, function* (jobs, existingStepMinutesByDate = {}) {
        ...
        debugLog(`Current Date: ${todayString}`);
        const orderedJobs = jobs.map((job) => (Object.assign(Object.assign({}, job), { job_steps: sortStepsByPriority(job.job_steps) })));
...
Do not include markdown formatting, code blocks, comments, or explanations.`;
        ...
        debugLog("---\n");
        try {
            const responseText = yield callAiTextResponse(provider, prompt, "batch job prioritization");
            ...
            debugLog(jsonString);
            const prioritySchedule = parseAiJsonArray(responseText, `${providerName} batch job prioritization`);
            ...
            const aiPriorityByJobId = new Map();
            const aiReasonByJobId = new Map();
            for (const item of prioritySchedule) {
                if (!Number.isInteger(item === null || item === void 0 ? void 0 : item.job_id)) {
                    console.warn(`Invalid prioritized job_id: ${item === null || item === void 0 ? void 0 : item.job_id}, skipping`);
                    continue;
                }
                ...
                if (!Number.isInteger(item === null || item === void 0 ? void 0 : item.priority) || item.priority <= 0) {
                    console.warn(`Invalid priority ${item === null || item === void 0 ? void 0 : item.priority} for job ${item === null || item === void 0 ? void 0 : item.job_id}, skipping`);
                    continue;
                }
                ...
                aiReasonByJobId.set(item.job_id, typeof item.reason === "string" && item.reason.trim().length > 0
                    ? item.reason.trim()
                    : "AI did not provide a reason; fallback scheduling rules will still apply.");
            }
            const prioritizedJobs = [...orderedJobs].sort((left, right) => {
                var _a, _b;
                const leftPriority = (_a = aiPriorityByJobId.get(left.job_id)) !== null && _a !== void 0 ? _a : Number.MAX_SAFE_INTEGER;
                const rightPriority = (_b = aiPriorityByJobId.get(right.job_id)) !== null && _b !== void 0 ? _b : Number.MAX_SAFE_INTEGER;
                if (leftPriority !== rightPriority) {
                    return leftPriority - rightPriority;
                }
            ...
            prioritizedJobs.forEach((job, index) => {
                var _a, _b;
                const priorityLabel = (_a = aiPriorityByJobId.get(job.job_id)) !== null && _a !== void 0 ? _a : "fallback";
                const reasonLabel = (_b = aiReasonByJobId.get(job.job_id)) !== null && _b !== void 0 ? _b : "Fallback by due date and job number.";
            ...
            debugLog(`${providerName} batch prioritization + deterministic scheduling: Total ${completedTotalPlanned} units planned across ${completedPlans.length} planning records`);
            debugLog("\nFinal Batch Planning Pairs:");
            completedPlans.slice(0, 10).forEach((plan, index) => {
                debugLog(`  ${index + 1}. Job ID: ${plan.job_id}, Date: ${plan.date}, Step ID: ${plan.job_step_id}, Quantity: ${plan.quantity}`);
            });
            ...
            return {
                ...
                priorityRecommendations: prioritizedJobs.map((job) => {
                    ...
                    return ({
                        job_id: job.job_id,
                        job_number: job.job_number,
                        priority: (_a = aiPriorityByJobId.get(job.job_id)) !== null && _a !== void 0 ? _a : null,
                        reason: (_b = aiReasonByJobId.get(job.job_id)) !== null && _b !== void 0 ? _b : "Fallback by due date and job number.",
                    });
                }),
            };
        }
